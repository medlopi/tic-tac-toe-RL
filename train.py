        ...
        try:
            for i in range(self.game_batch_num):
                ...
                if (i+1) % self.check_freq == 0:
                    ...
                    self.policy_value_net.save_model(f'./current_policy_{self.board_width}x{self.board_height}x{self.n_in_row}.model')
                    if win_ratio > self.best_win_ratio:
                        print("New best policy!!!!!!!!")
                        self.best_win_ratio = win_ratio
                        # update the best_policy
                        self.policy_value_net.save_model(f'./best_policy_{self.board_width}x{self.board_height}x{self.n_in_row}.model')
