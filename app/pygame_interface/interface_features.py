...
class Figure:
    ...
    def _render(self, size: int):
        ...
        shape_rect = pygame.Rect(
            rect_margin, rect_margin, render_area_size, render_area_size
        )

        if player_bit == Player.Type.CROSS.value:
            line_points = [
                (shape_rect.topleft, shape_rect.bottomright),
                (shape_rect.topright, shape_rect.bottomleft),
            ]
        ...
        if has_center_square_outline:
            ...
            pygame.draw.rect(
                surf, center_square_color, square_rect, max(1, thickness // 3)
            )
    ...
    def draw(
        ...
        if display_rect:
            ...
            target_pos = (
                center_pos[0] - current_display_size // 2,
                center_pos[1] - current_display_size // 2,
            )
        else:
        ...
        scaled_image = pygame.transform.smoothscale(
            self.image, (current_display_size, current_display_size)
        )
...
class PyGameInterfaceFeatures:
    ...
    def __init__(
        ...
        self.ai_opponent_for_human = (
            self.az_player if self.human_plays_against_ai else None
        )
        ...
        self.inventory_panel_width = (self.inv_piece_size * self.inv_columns) + (
            PADDING * (self.inv_columns + 1)
        )
        self.inventory_panel_width = max(
            self.inventory_panel_width, self.INV_PANEL_MIN_WIDTH
        )
    ...
    def _init_inventories(self):
        ...
        self.inventory = {
            Player.Type.CROSS: [
                Figure(c, Field.COUNT_FEATURES) for c in range(count_figures_per_player)
            ],
            Player.Type.NAUGHT: [
                Figure(c, Field.COUNT_FEATURES)
                for c in range(count_figures_per_player, 2 * count_figures_per_player)
            ],
        }

    def _calculate_base_cell_size(self):
        screen_w, screen_h = self.initial_size
        current_inv_panel_width = (self.inv_piece_size * self.inv_columns) + (
            PADDING * (self.inv_columns + 1)
        )
    ...
    def _calculate_inv_piece_size(self, available_height_for_inv=None):
        if available_height_for_inv is None:
            available_height_for_inv = (self.initial_size[1]) - (
                STATUS_HEIGHT + MESSAGE_HEIGHT + 2 * PADDING
            )

        num_pieces_per_player = 1 << (Field.COUNT_FEATURES - 1)
        if num_pieces_per_player == 0:
            return self.MIN_INV_PIECE_SIZE

        size_one_col = (
            available_height_for_inv - (num_pieces_per_player + 1) * PADDING
        ) / num_pieces_per_player

        self.inv_columns = 1
        calculated_size = size_one_col
        if (
            num_pieces_per_player > self.MAX_PIECES_PER_INV_COLUMN
            and size_one_col < self.MIN_INV_PIECE_SIZE * 1.5
        ):
            potential_cols = 2
            rows_per_column_multi = (
                num_pieces_per_player + potential_cols - 1
            ) // potential_cols
            if rows_per_column_multi == 0:
                rows_per_column_multi = 1  # Avoid division by zero
            size_multi_col = (
                available_height_for_inv - (rows_per_column_multi + 1) * PADDING
            ) / rows_per_column_multi
        ...
        final_size = max(
            self.MIN_INV_PIECE_SIZE, min(int(calculated_size), self.MAX_INV_PIECE_SIZE)
        )
        return final_size
    ...
    def _init_window(self):
        ...
        pygame.display.set_caption(
            f"MxNxKxD Game ({Field.WIDTH}x{Field.HEIGHT}x{Field.STREAK_TO_WIN}x{Field.COUNT_FEATURES})"
        )
    ...
    def _resize(self):
        ...
        self.available_inv_height = screen_height - (
            STATUS_HEIGHT + MESSAGE_HEIGHT + 2 * PADDING
        )
        ...
        self.left_inventory_rect = pygame.Rect(
            PADDING,
            STATUS_HEIGHT + MESSAGE_HEIGHT + PADDING,
            self.inventory_panel_width,
            self.available_inv_height,
        )
        self.right_inventory_rect = pygame.Rect(
            screen_width - PADDING - self.inventory_panel_width,
            STATUS_HEIGHT + MESSAGE_HEIGHT + PADDING,
            self.inventory_panel_width,
            self.available_inv_height,
        )
            ...
            if self.mcts_vs_dqn:
                if not self.game_over and calc_thread is None:
                    ...
                    def calculate_move_ai_vs_ai():
                        ...
                        if who_moves == Player.Type.CROSS:
                            if self.mcts_vs_dqn_choice == "mcts_x":
                                if self.pure_mcts_player:
                                    next_move = self.pure_mcts_player.get_move()
                            else:
                                if self.az_player:
                                    next_move = self.az_player.get_move()
                    ...
                    if self.az_player or self.pure_mcts_player:
                        calc_thread = threading.Thread(
                            target=calculate_move_ai_vs_ai, daemon=True
                        )
                        calc_thread.start()
                if calc_thread and not calc_thread.is_alive():
                    if next_move is not None:
                        ...
                        piece_to_remove = next(
                            (p for p in inventory_list if p.code == next_move.figure),
                            None,
                        )
                        if piece_to_remove:
                            inventory_list.remove(piece_to_remove)
                        self.game.make_silent_move(next_move)
                        if self.az_player:
                            self.az_player.move_and_update(next_move)
                        if self.pure_mcts_player:
                            self.pure_mcts_player.move_and_update(next_move)
                        self._update_game_state()
                        next_move = None
                    calc_thread = None
                ...
                if (
                    not self.game_over
                    and not calc_thread
                    and self.ai_opponent_for_human
                ):
                    ...
                    calc_thread = threading.Thread(target=worker_ai_move, daemon=True)
                    calc_thread.start()
                if calc_thread and not calc_thread.is_alive():
                    if next_move is not None:
                        moving_player_type = self.game.current_state.who_moves
                        ...
                        if piece_to_remove:
                            inventory_list.remove(piece_to_remove)
                        self.game.make_silent_move(next_move)
                        if self.ai_opponent_for_human:
                            self.ai_opponent_for_human.move_and_update(next_move)
                        self._update_game_state()
                        next_move = None
                    calc_thread = None
            self._draw()
    ...
    def _handle_event(self, event):
        ...
        if event.type == MOUSEBUTTONDOWN:
            if event.button == 1 and can_human_interact:
                active_inventory_rect = (
                    self.left_inventory_rect
                    if current_player_board_turn == Player.Type.CROSS
                    else self.right_inventory_rect
                )
                if not self.dragged_piece and active_inventory_rect.collidepoint(
                    event.pos
                ):
                    for piece in self.inventory[current_player_board_turn]:
                        if (
                            piece._inventory_rect
                            and piece._inventory_rect.collidepoint(event.pos)
                        ):
            ...
            if (
                event.button == 1
                and not self.dragged_piece
                and self.field_rect.collidepoint(event.pos)
            ):
                if self.game_over:
                    self._reset()
            ...
            if event.button == 1 and self.dragged_piece:
                if self.field_rect.collidepoint(event.pos) and can_human_interact:
                    ...
                    if (
                        ...
                        self.inventory[current_player_board_turn].remove(
                            self.dragged_piece
                        )
    ...
    def _zoom(self, mouse_screen_pos, factor):
        mouse_field_x = mouse_screen_pos[0] - self.field_x
        mouse_field_y = mouse_screen_pos[1] - self.field_y
        world_x = (mouse_field_x - self.view_offset_x) / self.zoom
        world_y = (mouse_field_y - self.view_offset_y) / self.zoom
        self.zoom = max(self.min_zoom, min(self.max_zoom, self.zoom * factor))
        self.view_offset_x = mouse_field_x - world_x * self.zoom
        self.view_offset_y = mouse_field_y - world_y * self.zoom

    def center_view(self):
        board_pixel_width = Field.WIDTH * self.cell_size
        board_pixel_height = Field.HEIGHT * self.cell_size
        self.view_offset_x = (self.field_width - board_pixel_width) / 2
        self.view_offset_y = (self.field_height - board_pixel_height) / 2
    ...
    def _to_cell(self, screen_pos):
        relative_x_to_field_surf = screen_pos[0] - self.field_x
        relative_y_to_field_surf = screen_pos[1] - self.field_y
        if self.cell_size == 0:
            return -1, -1
    ...
    def _update_game_state(self):
        ...
        if state == GameStates.CONTINUE:
            ...
            self.game_over = True
            self.game_msg = {
                GameStates.CROSS_WON: "Crosses Victory! Click/R to restart",
                GameStates.NAUGHT_WON: "Noughts Victory! Click/R to restart",
                GameStates.TIE: "Tie! Click/R to restart",
            }[state]
    ...
    def _draw(self):
        ...
        self._draw_status()
        self._draw_message()
        self._draw_inventories()
        self._draw_field()
        if self.game_over and self.win_line:
            self._draw_win_line()
        if self.dragged_piece:
            self.dragged_piece.draw(
                self.screen,
                center_pos=self.dragged_piece.pos,
                display_size=self.inv_piece_size,
            )
        if self.game_over:
    ...
    def _draw_single_inventory(self, pieces_list, panel_rect, is_active_player_inv):
        ...
        for col_idx in range(self.inv_columns):
            x_offset_col = (
                panel_rect.left + PADDING + col_idx * (self.inv_piece_size + PADDING)
            )
            ...
            for row_idx_in_col in range(rows_per_visual_column):
                ...
                piece_rect = pygame.Rect(
                    x_offset_col, y_offset, self.inv_piece_size, self.inv_piece_size
                )
                piece.draw(self.screen, display_rect=piece_rect)
                piece._inventory_rect = piece_rect
                if is_active_player_inv:
                    pygame.draw.rect(self.screen, (255, 255, 0, 100), piece_rect, 2)
                y_offset += self.inv_piece_size + PADDING
                current_piece_index += 1

    def _draw_inventories(self):
        pygame.draw.rect(self.screen, COLOR_INV_BG, self.left_inventory_rect)
        current_player_board_turn = self.game.current_state.who_moves
        is_human_turn = self.allowed_to_click
        self._draw_single_inventory(
            self.inventory[Player.Type.CROSS],
            self.left_inventory_rect,
            is_human_turn and current_player_board_turn == Player.Type.CROSS,
        )
        pygame.draw.rect(self.screen, COLOR_INV_BG, self.right_inventory_rect)
        self._draw_single_inventory(
            self.inventory[Player.Type.NAUGHT],
            self.right_inventory_rect,
            is_human_turn and current_player_board_turn == Player.Type.NAUGHT,
        )
    ...
    def _draw_status(self):
        ...
        self.screen.blit(
            text,
            (
                self.status_rect.left + PADDING,
                (self.status_rect.height - text.get_height()) // 2,
            ),
        )
    ...
    def _draw_message(self):
        ...
        text_x = (self.message_rect.width - text.get_width()) // 2
        text_y = (
            self.message_rect.top + (self.message_rect.height - text.get_height()) // 2
        )
        self.screen.blit(text, (text_x, text_y))

    def _draw_field(self):
        if not hasattr(self, "field_surf"):
            return
        ...
        max_row = min(
            Field.HEIGHT,
            int((-self.view_offset_y + self.field_height) / self.cell_size) + 1,
        )
        min_col = max(0, int(-self.view_offset_x / self.cell_size) - 1)
        max_col = min(
            Field.WIDTH,
            int((-self.view_offset_x + self.field_width) / self.cell_size) + 1,
        )
        for row in range(min_row, max_row):
            for col in range(min_col, max_col):
                val = self.game.current_state.field[row][col]
                cell_rect_fs = pygame.Rect(
                ...
                pygame.draw.rect(
                    ...
                    COLOR_GRID,
                    cell_rect_fs,
                    width=max(1, int(self.cell_size * 0.03)),
                )
                if val != -1:
                    piece_to_draw = Figure(val, Field.COUNT_FEATURES)
                    piece_to_draw.draw(
                        self.field_surf,
                        center_pos=cell_rect_fs.center,
                        display_size=int(self.cell_size * 0.85),
                    )

    def _draw_win_line(self):
        if not self.win_line or not hasattr(self, "field_surf") or self.cell_size == 0:
            return
        start_row, start_col, end_row, end_col = self.win_line
        progress = min(
            1.0, (pygame.time.get_ticks() - self.game_over_start_time) / 800.0
        )
        half_cell = self.cell_size / 2
        x1_fs = self.view_offset_x + start_col * self.cell_size + half_cell
        y1_fs = self.view_offset_y + start_row * self.cell_size + half_cell
        x2_fs = self.view_offset_x + end_col * self.cell_size + half_cell
        y2_fs = self.view_offset_y + end_row * self.cell_size + half_cell
        current_x_fs = x1_fs + (x2_fs - x1_fs) * progress
        current_y_fs = y1_fs + (y2_fs - y1_fs) * progress
        pygame.draw.line(
            self.field_surf,
            COLOR_WIN_LINE,
            (x1_fs, y1_fs),
            (current_x_fs, current_y_fs),
            max(4, int(self.cell_size * 0.1)),
        )
    ...
    def _draw_game_over_overlay(self):
        ...
        text_parts = self.game_msg.split("!")
        main_msg = text_parts[0] + ("!" if len(text_parts) > 1 else "")
        sub_msg = (
            text_parts[1].strip()
            if len(text_parts) > 1
            else "Click or Press R to restart"
        )
        ...
        for i, line_text in enumerate(lines_to_draw):
            ...
            text_rect = text_surf.get_rect(
                center=(
                    ...
                    self.screen.get_height() // 2 + (i * 60) - 30,
                )
            )
    ...
    def _reset(self):
        ...
        if self.mcts_vs_dqn:
            ...
            if self.pure_mcts_player:
                self.pure_mcts_player.reset_player()
    ...
    def _find_win_line(self):
        ...
        if placed_piece_code == -1:
            return None
        if (
            hasattr(self.game.current_state, "winning_line_info")
            and self.game.current_state.winning_line_info
        ):
            return self.game.current_state.winning_line_info
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        for dr, dc in directions:
            line = self._check_dir_visual_streak(
                field, last_move.row, last_move.col, dr, dc, placed_piece_code
            )
            if line:
                return line
        return None

    def _check_dir_visual_streak(
        self, field_array, r_start, c_start, dr, dc, target_code
    ):
        line_coords = []
        curr_r, curr_c = r_start, c_start
        while (
            0 <= curr_r < Field.HEIGHT
            and 0 <= curr_c < Field.WIDTH
            and field_array[curr_r][curr_c] == target_code
        ):
