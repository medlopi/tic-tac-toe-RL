...
def measure_mcts_performance(func):
    ...
    def wrapper(*args, **kwargs):
        if not hasattr(args[0], '_run_playout'):
            raise WrongMethodError("Декоратор measure_mcts_performance может использоваться только на методах класса MCTS")
        ...
        def new_node_select_action(self, puct_constant):
            ...
            if node is not None:
                ...
                if node_id not in mcts_stats['node_values']:
                    mcts_stats['node_values'][node_id] = {
                        'visits': 0,
                        'value': 0
                    }
        ...
        if mcts_stats['total_nodes'] > 0:
            ...
            avg_ucb = np.mean(mcts_stats['ucb_values']) if mcts_stats['ucb_values'] else 0
            avg_sim_time = np.mean(mcts_stats['simulation_times']) if mcts_stats['simulation_times'] else 0
            win_rate = (mcts_stats['winning_simulations'] / mcts_stats['total_simulations'] * 100) if mcts_stats['total_simulations'] > 0 else 0
            avg_branching = np.mean(mcts_stats['branching_factors']) if mcts_stats['branching_factors'] else 0
