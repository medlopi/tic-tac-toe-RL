...
class PyGameInterface:
    def __init__(self,dqn_player, mcts_enabled : bool, player_type : Player.Type, game=None, fullscreen_start=False, initial_size=None, mcts_vs_dqn=False, mcts_vs_dqn_choice='mcts_x'):
        ...
        self.zoom = 1.0
        self.min_zoom, self.max_zoom = 0.2, 5.0  # Ограничения зума и тп: теперь поле можно перетаскивать и приближать
    ...
    def handle_resize(self):
        ...
        self.field_height = screen_height - (STATUS_HEIGHT + MESSAGE_HEIGHT + PADDING * 2)
        self.field_x = PADDING
        self.field_y = STATUS_HEIGHT + MESSAGE_HEIGHT + PADDING
        self.field_surface_rect = pygame.Rect(self.field_x, self.field_y, self.field_width, self.field_height)
    ...
    def handle_event(self, event):
        ...
        if event.type == MOUSEBUTTONDOWN and event.button == 1:
            menu_button_rect = pygame.Rect(self.screen.get_width() - MENU_BUTTON_WIDTH - 10, 5, MENU_BUTTON_WIDTH, MENU_BUTTON_HEIGHT)
    ...
    def draw_status_bar(self):
        ...
        menu_button_rect = pygame.Rect(screen_width - MENU_BUTTON_WIDTH - 10, 5, MENU_BUTTON_WIDTH, MENU_BUTTON_HEIGHT)
    ...
    def draw_message_panel(self):
        screen_width = self.screen.get_width()
        pygame.draw.rect(self.screen, COLOR_MESSAGE, (0, STATUS_HEIGHT, screen_width, MESSAGE_HEIGHT))
        font = pygame.font.SysFont('Arial', 24)
    ...
    def draw_game_field(self):
        ...
        end_col = min(Field.WIDTH, int((-self.view_offset_x + self.field_width) / cell_size) + 1)
        start_row = max(0, int(-self.view_offset_y / cell_size))
        end_row = min(Field.HEIGHT, int((-self.view_offset_y + self.field_height) / cell_size) + 1)
    ...
    def draw_cell(self, surface, x, y, row, col):
        ...
        if hasattr(self.game.current_state, 'last_move') and self.game.current_state.last_move.row != -1:
            if row == self.game.current_state.last_move.row and col == self.game.current_state.last_move.col:
        ...
        if cell_content == Player.Type.CROSS:
            ...
            if radius > 0:
                pygame.draw.circle(surface, COLOR_O_OUTLINE, center, radius, outline_thickness)
                pygame.draw.circle(surface, COLOR_O_MAIN, center, radius, main_thickness)
    ...
    def draw_win_line(self):
        ...
        pygame.draw.line(self.field_surface, COLOR_WIN_LINE, (x1, y1), (current_x, current_y), max(4, int(cell_size * 0.1)))
    ...
    def draw_game_over(self):
        ...
        if len(lines) > 1: lines = [lines[0] + "!", lines[1]]
        else: lines = [self.game_msg, "Click to restart"]
        y_offset = -40
        for i, line in enumerate(lines):
            text = font.render(line, True, COLOR_TEXT)
            text_rect = text.get_rect(center=(self.screen.get_width()//2, self.screen.get_height()//2 + y_offset*i))
            self.screen.blit(text, text_rect)
